...
public class HttpClientIntegrationTests {
	...
	public void setup() {
		...
		this.httpClient = new HttpClient(new ReactorHttpClientRequestFactory());
	}
	...
	public void shouldGetPlainTextResponseAsObject() throws Exception {
		...
		result.subscribe(ts);
		ts.await(600).awaitAndAssertValues("Hello Spring!").assertComplete();
	...
	@Test
	@Ignore("Fails: not emitting any signal")
	public void shouldGetJsonAsListOfPojos() throws Exception {
		...
		TestSubscriber<List<Pojo>> ts = new TestSubscriber();
		result.subscribe(ts);
		ts.awaitAndAssertValuesWith(
				l -> {
					assertThat(l.get(0).getBar(), Matchers.is("bar1"));
					assertThat(l.get(1).getBar(), Matchers.is("bar2"));
				}
		).assertValueCount(1).assertComplete();
		RecordedRequest request = server.takeRequest();
		assertEquals("/pojos", request.getPath());
		assertEquals("application/json", request.getHeader(HttpHeaders.ACCEPT));
	}
